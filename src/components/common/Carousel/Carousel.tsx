...
const Carousel = ({
  ...
  }, [activeItem, onActiveItemChanged]);
  ...
  useEffect(() => {
    if (automaticSwipe && !everTouched) {
      const timer = setTimeout(
        () =>
          setActiveItem(
            activeItem + 1 < listOfchildren.length ? activeItem + 1 : 0
          ),
        3000
      );
      return () => clearTimeout(timer);
    }
  }, [everTouched, activeItem, listOfchildren.length, automaticSwipe]);
